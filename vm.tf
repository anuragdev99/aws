    ...
    mainSteps     = [
      {
        ...
        inputs = {
          runCommand = [
            ...
            "        }",
            "      }",
            "      $volume = Get-Volume -DiskNumber $disk.Number | Where-Object FileSystem -eq $null",
            "      if ($volume) {",
            "        Format-Volume -Partition $partition -FileSystem NTFS -NewFileSystemLabel 'sree' -Force -Confirm:$false | Out-Null",
            "      }",
